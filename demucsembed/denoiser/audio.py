...
def combine_embed (embed,signal):
    embed_lens = (torch.ones(embed.shape[0],1,1)*embed.shape[2]).to(signal.device)
    return torch.cat((embed_lens,embed,signal),2)

def extract_embed (signal_w_embed):
    embed_len = int(signal_w_embed[:,:,0][0])
    embed = signal_w_embed[:,:,1:embed_len+1]
    signal = signal_w_embed[:,:,embed_len+1:]
    return embed, signal
...
class Audioset:
    def __init__(self, files=None, length=None, stride=None,
        ...
        """
        files should be a list [(file, clean_file, embed_file, clean_file_length)]
        """
        self.files = files
        self.num_examples = []
        self.length = length
        ...
        for f_i,(noisy_file, clean_file, embed_file, file_length) in enumerate(self.files):
            if (self.set_type == "clean"):
                file = clean_file
            elif (self.set_type == "noisy" or self.with_path == True):
                file = noisy_file
            elif (self.set_type == "embed"):
                file = embed_file
            else:
    ...
    def __getitem__(self, index):
        start_index = index
        for (noisy_file, clean_file, embed_file, _), examples in zip(self.files, self.num_examples):
            if index >= examples:
                index -= examples
                continue
            ...
            if self.with_path:
                ...
                embed,sr = torchaudio.load(str(embed_file))
                return out, clean, embed, file
            else:
                return out
