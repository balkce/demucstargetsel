...
from .audio import Audioset, find_audio_files, extract_embed, combine_embed
...
def get_estimate(model, noisy, args):
    ...
    if args.streaming:
        ...
        with torch.no_grad():
            ...
            embed, noisy_wo_embed = extract_embed(noisy)
            #print("Get_estimate: "+str(noisy_wo_embed.shape)+" -> "+str(estimate.shape))
            estimate = (1 - args.dry) * estimate + args.dry * noisy_wo_embed
    return estimate


def save_wavs(estimates, noisy_sigs, clean_sigs, filenames, out_dir, sr=16_000):
    # Write result
...
def _estimate_and_save(model, noisy_signals, clean_signals, filenames, out_dir, args):
    estimate = get_estimate(model, noisy_signals, args)
    embed, noisy_wo_embed = extract_embed(noisy_signals)
    save_wavs(estimate, noisy_wo_embed, clean_signals, filenames, out_dir, sr=model.sample_rate)
...
def enhance(args, model=None, local_out_dir=None):
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        ...
        for data in iterator:
            # Get batch data
            noisy_signals, clean_signals, embed_signals, filenames = data
            noisy_signals = noisy_signals.to(args.device)
            embed_signals = embed_signals.to(args.device)
            
            embedding = torch.Tensor(embedding_extractor(embed_signals)).to(args.device).unsqueeze(1)
            noisy_embed = combine_embed(embedding,noisy_signals)
            
            if args.device == 'cpu' and args.num_workers > 1:
                pendings.append(
                    pool.submit(_estimate_and_save,
                                model, noisy_embed, clean_signals, filenames, out_dir, args))
            else:
                # Forward
                estimate = get_estimate(model, noisy_embed, args)
                save_wavs(estimate, noisy_signals, clean_signals, filenames, out_dir, sr=model.sample_rate)
