...
from .audio import extract_embed
...
class BLSTM(nn.Module):
    def __init__(self, dim, embed_dim, layers=2, bi=True):
        super().__init__()
        klass = nn.LSTM
        self.lstm = klass(bidirectional=bi, num_layers=layers, hidden_size=dim, input_size=dim+embed_dim)
        #self.lstm = klass(bidirectional=bi, num_layers=layers, hidden_size=dim+embed_dim, input_size=dim+embed_dim, proj_size=dim)
        self.linear = None
        if bi:
...
class Demucs(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = BLSTM(chin, self.embed_size, layers=self.lstm_depth, bi=not causal)
        if rescale:
    ...
    def forward(self, mix_embed):
        embed, mix = extract_embed(mix_embed)
        #print(str(mix_embed.shape)+" -> "+str(embed.shape)+", "+str(mix.shape))
        ...
        #concatenating embedding before lstm
        embed_cat = embed.repeat([1,x.shape[2],1])
        embed_cat = embed_cat.permute(0,2,1)
        x = th.cat((embed_cat,x),1)
        
        x = x.permute(2, 0, 1)
        x, _ = self.lstm(x)
        x = x.permute(1, 2, 0)
        
        #the lstm module projects to the
        #apropriate length for the following layers
...
class DemucsStreamer:
    ...
    Args:
        ...
        - resample_buffer (int): size of the buffer of previous inputs/outputs
            kept for resampling.
    """
    def __init__(self, demucs, embedding,
                 dry=0,
                 num_frames=1,
        ...
        assert (self.embedding.shape[0] == 1) and (self.embedding.shape[1] == self.demucs.embed_size), "embedding must be of size [1,"+str(self.demucs.embed_size)+"]"
    ...
    def _separate_frame(self, frame):
        ...
        #concatenating embedding before lstm
        embed_cat = self.embedding.repeat([1,x.shape[2],1]).to(self.device)
        embed_cat = embed_cat.permute(0,2,1)
        x = th.cat((embed_cat,x),1)
