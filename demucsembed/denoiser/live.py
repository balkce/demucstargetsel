...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Performs live speech enhancement, reading audio from "
                    ...
                    "Requires a JSON file with embedding information, for "
                    "target speech selection."
        )
    parser.add_argument(
        "-e", "--embedding", dest="embed_path",
        help="path to JSON file with embedding information.")
    parser.add_argument(
        "-i", "--in", dest="in_",
...
def main():
    ...
    if args.embed_path:
        json_embed_file = open(args.embed_path,"r")
        embedding_dict = json.load(json_embed_file)
        json_embed_file.close()
        
        embedding = torch.Tensor(embedding_dict['embedding']).unsqueeze(0)
    else:
        print("An embedding JSON is required. Use create_embeddings.py with a WAV recording to do so.")
        exit()
    ...
    model.eval()
    print("Model loaded.")
    streamer = DemucsStreamer(model, embedding, dry=args.dry, num_frames=args.num_frames)
