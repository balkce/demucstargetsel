...
from .audio import combine_embed
...
class Solver(object):
    def __init__(self, data, model, optimizer, args):
        ...
        self.embedding_extractor = PretrainedSpeakerEmbedding(
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            if ((epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1) and self.tt_loader:
                ...
                logger.info('-' * 70)
                if self.args.test_with_best:
                    logger.info('Evaluating on the test set with best model...')
                    # We switch to the best known model for testing
                    with swap_state(self.model, self.best_state):
                        pesq, stoi = evaluate(self.args, self.model, self.tt_loader)
                else:
                    logger.info('Evaluating on the test set with current model...')
                    # We use current model for testing
                    pesq, stoi = evaluate(self.args, self.model, self.tt_loader)
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            noisy, clean, embed = [x.to(self.device) for x in data]
            
            this_length = min(noisy.shape[2],clean.shape[2])
            noisy = noisy[:,:,:this_length]
            clean = clean[:,:,:this_length]
            
            #if embed.shape[2] >= this_length:
            #    embed = embed[:,:,:this_length]
            #else:
            #    embed = F.pad(embed, (0, this_length-embed.shape[2]))
            ...
            embedding = torch.Tensor(self.embedding_extractor(embed)).to(noisy.device).unsqueeze(1)
            noisy_embed = combine_embed(embedding,noisy)
            
            #noisy_embed = combine_embed(embed,noisy)
            
            estimate = self.dmodel(noisy_embed)
